...
TabShapeRLines <- function(Path,ImportName,ExportName){
  ...
  names(CombinedData)[names(CombinedData) == "PlotOrder"] <- "Path"
  names(CombinedData)[names(CombinedData) == "LineSubID"] <- "SubID"
  names(CombinedData)[names(CombinedData) == "LineID"] <- "ObjectID"
...
TabShapeRPoints <- function(Path,ImportName,ExportName){
  ...
  Data <- as(ShapeFile, "data.frame")
  Data$SubID <- as.numeric(rownames(Data))
  Data$ObjectID <- as.numeric(rownames(Data))
  Data$Path <- as.numeric(1)
...
TabShapeRPolygons <- function(Path,ImportName,ExportName){
  ...
  CombinedData <- merge(Data,coordinates)
  names(CombinedData)[names(CombinedData) == "PlotOrder"] <- "Path"
  names(CombinedData)[names(CombinedData) == "PolygonID"] <- "SubID"
